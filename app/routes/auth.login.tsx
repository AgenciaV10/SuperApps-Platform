...
import { useActionData, useNavigation, Link, useNavigate } from '@remix-run/react';
...
export const meta: MetaFunction = () => {
  return [{ title: 'Login - Super Apps' }, { name: 'description', content: 'Faça login na sua conta Super Apps' }];
};

export async function loader(_: LoaderFunctionArgs) {
  /*
   * Nota: A verificação de autenticação será feita no cliente
   * pois o Supabase Auth funciona principalmente no browser
   */
  return json({});
}
...
export async function action({ request }: ActionFunctionArgs) {
   ...
   * O login real será feito no cliente via JavaScript
   */
...
export default function Login() {
  ...
  const isSubmitting = navigation.state === 'submitting' || isLoggingIn;
  ...
  useEffect(() => {
    ...
    if (pendingPrompt) {
      console.log('Prompt pendente detectado, será restaurado após login');
    }
  }, []);

  // Função de login no cliente
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600">Faça login na sua conta</p>
        </div>
      </div>
